...
void basic_pointer::stream_insert(std::ostream& o) const
{
    o << "basic_pointer:" << d_underlying->name() << ':' << this;
}
...
void basic_value::stream_insert(std::ostream& o) const
{ o << "basic_value:" << name() << ':' << this; }
...
basic_array::basic_array(const basic_value * underlying, size_t num_elems)
    :
      type_descriptor(num_elems * underlying->type_size(), TYPE_BASIC_ARRAY,
                      underlying->name()),
      d_underlying(underlying),
...
void basic_array::stream_insert(std::ostream& o) const
{
    o << "basic_array:" << d_underlying->name() << '[' << d_num_elems << ']'
      << ':' << this;
}
...
{
    return
        lo ? global_refs::ptr->TRUE_object() : global_refs::ptr->FALSE_object();
}
